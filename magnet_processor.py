...
def save_batch_to_file(batch_data, headers, batch_num, output_dir, base_name):
    ...
    for row_idx, row_data in enumerate(batch_data, start=1):
        worksheet.write(row_idx, 0, row_data['magnet'])
        worksheet.write(row_idx, 1, row_data['name'])
        worksheet.write(row_idx, 2, row_data['count'])
        worksheet.write(row_idx, 3, row_data['size'])
        ...
        if row_data['temp_files']:
            ...
            for tmp_path in row_data['temp_files']:
                if os.path.exists(tmp_path):
                    ...
                    worksheet.insert_image(row_idx, col, tmp_path, {'x_scale': 1, 'y_scale': 1})
                    col += 1
...
def process_single_file(file_path, output_dir, progress_manager):
    ...
    if start_row > 0:
        print(f"  从第 {start_row + 1} 行继续（已处理 {start_row} 行）")
    
    try:
        wb_in = load_workbook(file_path, read_only=True)
        ws_in = wb_in.active
        
        first_row = next(ws_in.iter_rows(min_row=1, max_row=1, values_only=True), None)
        headers = []
        if first_row:
            headers = [str(h) if h else "" for h in first_row[:4]]
        else:
            headers = ["链接", "Resource Name", "Number of Files", "Total File Size"]
        ...
        for _ in range(start_row):
            try:
                next(ws_in.iter_rows(min_row=row_in_excel, max_row=row_in_excel, values_only=True))
                row_in_excel += 1
            except StopIteration:
                break
        
        for row in ws_in.iter_rows(min_row=row_in_excel, values_only=True):
            ...
            print(f"  处理第 {row_in_excel} 行 (批次 {batch_num}, 本次 {processed_in_this_run+1}/{max_rows_to_process})...")
            ...
            if len(batch_data) >= BATCH_SIZE:
                ...
                elapsed = time.time() - start_time
                print(f"  已处理 {start_row + processed_in_this_run}/{total_rows} 行，耗时 {elapsed/60:.1f} 分钟")
                print()
...
def main():
    ...
    for f in unprocessed_files:
        ...
        total = count_rows_in_file(f)
        print(f"  - {f.name} (已处理 {processed}/{total} 行)")
    print()
